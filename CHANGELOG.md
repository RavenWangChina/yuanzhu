...
- **通知中心页面**（/notifications）：推衍提议卡片（采纳/忽略）+ 自举开关（一键切换）+ 行为日志时间线 + 知识库统计
- **推衍引擎自动触发**：用户采纳答案后异步触发（不需要手动调接口）
...
- **真实步骤进度**：`POST /api/workflows/run-async` 立返 task_id + `GET /workflows/status/{id}` 轮询——Chat 显示真实"独立审查… 3/6 步"（替代定时器猜测的假进度）
- **排队提问**：思考中可继续输入（按钮变"排队"，完成后自动发送下一个）——不再锁输入框干等
- **流内洞见确认**：采纳后 AI 提议的洞见直接在对话流内显示（"沉淀吗？"[沉淀/跳过]）——不再跳去待审中心
- **模式检测可解释**：历史不足时显示一次性提示（"我会留意重复出现的问题…"）——用户知道此能力存在
- 附：dogfood P0 修复（aiMsg 响应式——Chat 界面真实可用性）+ 历史 adopted 按 adopted_by 判断 + 裁决防 ans_a 内部名泄漏

## [0.1.3] · 2026-09-11（个人工作台）

### Added（2C 个人用户体验四提升，145 测试全绿）
- **对话式问答界面**（Chat）：消息流+历史问答自动加载（本体即存储）+连续追问；首页 hero 带示例问题；阶段进度提示+中间过程折叠展开
- **差异可视化**："📚 已参考你 N 条历史洞见"提示——让记忆可见；结构化裁决展示
- **采纳即沉淀**：Web 采纳深度答案时自动触发洞见提炼（staged 轻确认）——用户不管理知识，知识自己长
- **重复模式检测（对话长出工作流）**：AI 分析问答历史发现例行公事 → 对话界面建议条"铸成工作流？"→ 一键 forge（历史作上下文）
- **CLI `yuanzhu ask "问题"`**：终端对话入口（--adopt 直接采纳）
- **不采纳也可用**（用户洞察）：答案默认入库（SaveAnswer L1 自动）为未采纳态——历史反哺/模式检测看**全部**问答，"采纳"降级为可信度升级标记（标记+触发洞见沉淀，走 /api/metaflow/adopt）
- 修复：vite 构建输出对齐包内资源（0.1.2 挪包遗留）
